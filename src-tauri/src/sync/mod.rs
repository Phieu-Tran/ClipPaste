...
use error::SyncError;
use models::{SyncRunReport, SyncSettings, SyncState, SyncStatus};
...
static SYNC_ABORT: Lazy<Mutex<Option<tokio::sync::watch::Sender<bool>>>> =
...
pub async fn get_sync_status(db: &Database) -> SyncStatus {
    ...
    let error_message = db.get_setting("sync_last_error").await.unwrap_or(None);
    let last_report = db
        .get_setting("sync_last_report")
        .await
        .unwrap_or(None)
        .and_then(|value| serde_json::from_str::<SyncRunReport>(&value).ok());
    let token_expires_at = db
        .get_setting("sync_token_expires_at")
        .await
        .unwrap_or(None)
...
async fn count_pending_changes(db: &Database, last_sync_at: &Option<String>) -> u64 {
    ...
    let clip_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM clips WHERE updated_at > ?")
        .bind(&since)
        .fetch_one(&db.pool)
        .await
        .unwrap_or(0);

    let folder_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM folders WHERE updated_at > ?")
        .bind(&since)
        .fetch_one(&db.pool)
        .await
        .unwrap_or(0);
    ...
    let tombstone_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM sync_tombstones WHERE deleted_at > ?")
            .bind(&since)
            .fetch_one(&db.pool)
            .await
            .unwrap_or(0);
...
pub async fn get_sync_settings(db: &Database) -> SyncSettings {
    let enabled = db
        ...
        .unwrap_or(false);
    let interval = db
        .get_setting("sync_interval_seconds")
        .await
        .unwrap_or(None)
        .and_then(|v| v.parse().ok())
        .unwrap_or(300u64);
    let sync_images = db
        .get_setting("sync_images")
        .await
        .unwrap_or(None)
        .map(|v| v != "false")
        .unwrap_or(true);
...
pub async fn cleanup_tombstones(db: &Database) -> Result<u64, sqlx::Error> {
    let result =
        sqlx::query("DELETE FROM sync_tombstones WHERE deleted_at < datetime('now', '-30 days')")
            .execute(&db.pool)
            .await?;
...
pub async fn get_valid_token(db: &Database) -> Result<String, SyncError> {
    let access_token = db
        .get_setting("sync_access_token")
        .await
        .map_err(|e| SyncError::Database(e.to_string()))?
        .ok_or(SyncError::NotConfigured)?;
    let refresh_token = db
        .get_setting("sync_refresh_token")
        .await
        .map_err(|e| SyncError::Database(e.to_string()))?
        .ok_or(SyncError::NotConfigured)?;
    ...
    if now > expires_at - 300 {
        ...
        save_setting(&db.pool, "sync_access_token", &new_tokens.access_token)
            .await
            .map_err(|e| SyncError::Database(e.to_string()))?;
        save_setting(&db.pool, "sync_refresh_token", &new_tokens.refresh_token)
            .await
            .map_err(|e| SyncError::Database(e.to_string()))?;
        save_setting(
            &db.pool,
            "sync_token_expires_at",
            &new_tokens.expires_at.to_string(),
        )
        .await
        .map_err(|e| SyncError::Database(e.to_string()))?;
...
pub fn get_encryption_key() -> Result<[u8; 32], SyncError> {
    ENCRYPTION_KEY.lock().ok_or(SyncError::Encryption(
        "Sync passphrase not unlocked. Please enter your passphrase in Settings > Sync.".into(),
    ))
}
...
pub async fn set_passphrase(db: &Database, passphrase: &str) -> Result<(), SyncError> {
    // Get or create salt
    let salt_hex = match db
        .get_setting("sync_encryption_salt")
        .await
        .map_err(|e| SyncError::Database(e.to_string()))?
    {
        Some(s) => s,
        None => {
            ...
            let hex = salt
                ...
                .map(|b| format!("{:02x}", b))
                .collect::<String>();
            save_setting(&db.pool, "sync_encryption_salt", &hex)
                .await
                .map_err(|e| SyncError::Database(e.to_string()))?;
            hex
        }
    ...
    save_setting(&db.pool, "sync_passphrase_hash", &hash)
        .await
        .map_err(|e| SyncError::Database(e.to_string()))?;
...
pub async fn unlock_with_passphrase(db: &Database, passphrase: &str) -> Result<bool, SyncError> {
    let hash = db
        .get_setting("sync_passphrase_hash")
        .await
        .map_err(|e| SyncError::Database(e.to_string()))?
...
pub async fn execute_sync(db: &Database) -> Result<String, String> {
    ...
    let result: Result<String, String> = async {
        ...
        let report = protocol::sync_now(db, &drive, settings.sync_images)
            .await
        ...
        let message = if report.skipped {
            ...
            format!(
                "Synced: pushed {}/{} clips/folders, pulled {}/{}, deleted {}",
                report.pushed_clips,
        ...
        let run_report = SyncRunReport {
            pushed_clips: report.pushed_clips,
            pushed_folders: report.pushed_folders,
            pulled_clips: report.pulled_clips,
            pulled_folders: report.pulled_folders,
        ...
        save_setting(
            &db.pool,
            "sync_last_report",
            &serde_json::to_string(&run_report).unwrap_or_default(),
        )
        .await
        .map_err(|e| e.to_string())?;
        let _ = sqlx::query("DELETE FROM settings WHERE key = 'sync_last_error'")
            .execute(&db.pool)
            .await;
        Ok(message)
    }
    ...
    if let Err(error) = &result {
        let _ = save_setting(&db.pool, "sync_last_error", error).await;
    }
