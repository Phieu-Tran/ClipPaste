...
describe('ClipList', () => {
  ...
  it('has role="listbox" on scroll container', () => {
    const { container } = render(<ClipList {...defaultProps} />);
  ...
  it('has aria-label on listbox', () => {
    const { container } = render(<ClipList {...defaultProps} />);
    const listbox = container.querySelector('[role="listbox"]');
    expect(listbox?.getAttribute('aria-label')).toBe('Clipboard history');
  });

  it('has aria-orientation="horizontal" on listbox', () => {
    const { container } = render(<ClipList {...defaultProps} />);
    const listbox = container.querySelector('[role="listbox"]');
    expect(listbox?.getAttribute('aria-orientation')).toBe('horizontal');
  });

  // === Improved empty state tests ===
  ...
  it('shows search-specific empty text when searching', () => {
    render(<ClipList {...defaultProps} clips={[]} isSearching={true} />);
